 ...
 */
async function addPendingEntry({ userId, familyId, source, rawText, parsedData }) {
...
/**
 * List all pending entries for a user/family
 */
async function listPendingEntries({ userId, familyId }) {
  const db = await getDb();
 ...
 */
async function countPendingEntries({ userId, familyId }) {
  const db = await getDb();
  const query = { status: "pending" };
  if (familyId) {
    query.family_id = familyId;
  ...
  return await pendingCollection(db).countDocuments(query);
}
