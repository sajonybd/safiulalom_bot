 ...
 */
async function addEntity({ userId, familyId, name, type, subType, groupId, phone, email, metadata = {} }) {
...
async function updateEntity({ familyId, id, updates }) {
  ...
  if (updates.phone !== undefined) payload.phone = updates.phone ? String(updates.phone).trim() : null;
  if (updates.email !== undefined) payload.email = updates.email ? String(updates.email).trim() : null;
 ...
 */
async function findEntityByName({ familyId, name, type }) {
  const db = await getDb();
  const query = {
    family_id: familyId,
  ...
  const doc = await entitiesCollection(db).findOne(query);
  return doc ? mapEntityShape(doc) : null;
}
