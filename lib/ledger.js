...
async function updateEntry({ familyId, id, amount, note, person, entityId, metadata, createdAt }) {
  const db = await getDb();
...
async function accountsBalances({ familyId }) {
  ...
  for (const row of rows) {
    ...
    if (row.destination_account && ["in", "fund_received", "loan_taken", "settlement_in", "person_in"].includes(row.kind)) {
      balances[row.destination_account] = (balances[row.destination_account] || 0) + amt;
    }
    
    // Outflow from source account
    if (row.source_account && ["out", "sub", "loan_given", "settlement_out", "person_out"].includes(row.kind)) {
      balances[row.source_account] = (balances[row.source_account] || 0) - amt;
    }
    
    // Transfer logic
    if (row.kind === "transfer" && row.source_account && row.destination_account) {
      balances[row.destination_account] = (balances[row.destination_account] || 0) + amt;
      balances[row.source_account] = (balances[row.source_account] || 0) - amt;
    }
  }
