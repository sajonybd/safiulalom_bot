...
async function addEntry({
  ...
  if (!chatId) {
    const { notifyTransaction } = require("./notifications");
    void notifyTransaction(userId, { ...doc, id: result.insertedId }).catch((e) => {
      console.error("[Ledger] Notification failed:", e.message);
    });
  }
...
async function updateEntry({ familyId, id, kind, amount, note, person, sourceAccount, destinationAccount, entityId, metadata, createdAt }) {
  ...
  if (result && !result.chat_id) {
    const { notifyTransaction } = require("./notifications");
    void notifyTransaction(result.user_id, result).catch((e) => {
      console.error("[Ledger] Update notification failed:", e.message);
    });
  }
