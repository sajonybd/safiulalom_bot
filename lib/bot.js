...
function getBot() {
  ...
  const showHelp = authGuard(async (ctx) => {
    ...
    await ctx.reply(
      [
        ...
        "",
        "🚀 *Commands:*",
        "• `/acc` - Quick financial snapshot",
        "• `/s` - Monthly total summary",
        "• `/credit` - Check AI remaining bits",
        "• `/login` - Simple dashborad access",
      ].join("\n"),
  ...
  bot.command(["ui", "login"], handleLoginRequest);
  ...
  bot.telegram.setMyCommands([
    ...
    { command: "acc", description: "🏦 Wallet Balances" },
    { command: "s", description: "📊 Monthly Summary" },
    { command: "credit", description: "💳 AI Bits status" },
    { command: "help", description: "📖 Assistant Guide" },
  ]).catch(() => {});
