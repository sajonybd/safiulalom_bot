...
1) EXPLICIT: Use the account mentioned in the text (e.g., "bKash", "Bank").
2) PREFERENCE: If not mentioned, check user preferences (e.g., if it's a "salary", check "salaryAccount").
...
async function getAssistantContext({ userId, familyId, source }) {
  ...
  return {
    ...
    chatHistory: chatHistory.reverse().map(m => ({
      ...
      parts: [{ text: (m.role === "assistant" && m.metadata?.raw_response) ? m.metadata.raw_response : m.content }]
    }))
  };
}
...
- The JSON inside <action> MUST be complete and valid. 
- NO MARKDOWN or explanations outside <action> tags for JSON.
- NEVER start with "tags:*" or "json:*".
